...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        output = np.maximum(0, input.dot(W1) + b1)
        ...
        ############################################################################
        max_f_i = np.max(scores, axis=1)
        f_i = (scores.T - max_f_i.T).T
        sum_f_i = np.sum(np.exp(f_i), axis=1)
        l_i = -f_i[np.arange(N), y] + np.log(sum_f_i)
        loss = np.sum(l_i)
        loss /= N
        loss += 0.5 * self.reg * (np.sum(W1 * W1) + np.sum(W2 * W2))

        p = ((np.exp(f_i)).T / sum_f_i).T
        p[np.arange(N), y] -= 1
        dW = p / N
        grads['W2'] = output.T.dot(dW) + self.reg * W2
        grads['b2'] = np.sum(dW, axis=0)
        dh = dW.dot(W2.T)
        dh_Relu = (output > 0) * dh
        grads['W1'] = input.T.dot(dh_Relu) + self.reg * W1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for index, hidden_dim in enumerate(hidden_dims, 1):
            self.params['W{}'.format(index)] = weight_scale * np.random.randn(layer_input_dim, hidden_dim)
            self.params['b{}'.format(index)] = np.zeros(hidden_dim)
